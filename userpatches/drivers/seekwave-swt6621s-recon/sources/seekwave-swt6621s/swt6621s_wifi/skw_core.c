...
static int skw_core_init(struct skw_core *skw, struct platform_device *pdev, int idx)
{
	...
	skw_chip_info(skw->idx,
		"SWT6621S probe: core_init idx=%d bus=%d bus_type=0x%x data_port=%d cmd_port=%d hw_adma_tx=%ps hw_sdma_tx=%ps\n",
		idx, skw->hw.bus, skw->hw_pdata->bus_type, skw->hw_pdata->data_port,
		skw->hw_pdata->cmd_port, skw->hw_pdata->hw_adma_tx,
		skw->hw_pdata->hw_sdma_tx);
...
static int skw_add_default_iface(struct wiphy *wiphy)
{
	...
	skw_chip_info(skw->idx, "SWT6621S probe: add_default_iface enter\n");
	...
	if (IS_ERR(iface)) {
		...
		skw_chip_err(skw->idx,
			"SWT6621S probe: add_default_iface station failed ret=%d\n",
			ret);
	...
	skw_chip_info(skw->idx,
		"SWT6621S probe: add_default_iface station ok ifname=%s id=%d iftype=%d\n",
		iface->ndev ? netdev_name(iface->ndev) : "none", iface->id,
		iface->wdev.iftype);
...
unlock:
	...
	skw_chip_info(skw->idx, "SWT6621S probe: add_default_iface exit ret=%d\n", ret);
...
static int skw_drv_probe(struct platform_device *pdev)
{
	...
	skw_chip_info(idx, "SWT6621S probe: drv_probe enter dev=%s\n",
		dev_name(&pdev->dev));
	...
	skw_chip_info(skw->idx, "SWT6621S probe: drv_probe core_init ok\n");
	...
	skw_chip_info(skw->idx,
		"SWT6621S probe: drv_probe chip_info ok chip_model=0x%x fw_version=0x%x\n",
		chip.chip_model, chip.fw_version);
	...
	skw_chip_info(skw->idx, "SWT6621S probe: drv_probe setup_wiphy ok\n");
	...
	skw_chip_info(skw->idx, "SWT6621S probe: drv_probe default iface ok\n");
	...
	skw_chip_info(skw->idx, "SWT6621S probe: drv_probe success\n");
