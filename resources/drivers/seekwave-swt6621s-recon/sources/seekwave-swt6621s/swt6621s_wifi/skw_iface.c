				...
				u8 id, bool need_ndev)
{
	...
	skw_chip_info(skw->idx,
		"SWT6621S probe: add_iface name=%s inst=%d iftype=%d need_ndev=%d\n",
		name, inst, iftype, need_ndev);
	...
	if (need_ndev) {
		...
		skw_chip_info(skw->idx,
			"SWT6621S probe: add_iface alloc_netdev ok name=%s\n",
			name);
	...
	if (ndev) {
		...
		if (ret) {
			...
			skw_chip_err(skw->idx,
				"SWT6621S probe: add_iface netdev_init failed ret=%d\n",
				ret);
			goto iface_teardown;
		}
		...
		if (ret) {
			...
			skw_chip_err(skw->idx,
				"SWT6621S probe: add_iface register_netdevice failed ret=%d ifname=%s\n",
				ret, ndev->name);
		...
		skw_chip_info(skw->idx,
			"SWT6621S probe: add_iface register_netdevice ok ifname=%s dev_addr=%pM\n",
